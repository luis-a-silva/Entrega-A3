...
async function fetchData() {
    try {
        ...
        const response = await fetch(api + '/getVendedor');
        ...
        data.forEach(item => {
            ...
            cellMatricula.textContent = item.cod_matricula;
            ...
            acoesBtn.innerHTML = `<button onclick="btnDeleteHandler(${item.id})" class="icon" title="Apagar"><i class="material-icons">delete</i></button><button id=editar onclick="btnEditarHandler(${item.id})" class="icon" title="Editar"><i class="material-icons">edit</i></button>`
...
const clienteData = {
    cod_matricula: document.getElementById('cod_matricula').value,
...
postData(api + '/addVendedor', clienteData); // Altere a URL conforme a necessidade
});


function btnDeleteHandler(item){
    const id =  item;
    var confirmDelete = confirm('Deseja deletar vendedor? ');
    if(confirmDelete) {
        deleteData(api + "/deleteVendedor", id);
    } else {
...
async function btnEditarHandler(id) {
    ...
    try {
        // Faça a requisição para a API
        const response = await fetch(`${api}/getVendedorById/${id}`);
        ...
        data.forEach(item => {
            console.log(item.cod_matricula);
            ...
            document.getElementById("editarCod_matricula").value = item.cod_matricula; // Preencher nome
    ...
    document.getElementById('clienteEditForm').addEventListener('submit', function(event) {
        ...
        const editClienteData = {
            cod_matricula: document.getElementById('editarCod_matricula').value,
        ...
        // Chama a função postData para enviar os dados
        putData(api + '/updateVendedor',id, editClienteData);
    });
}
